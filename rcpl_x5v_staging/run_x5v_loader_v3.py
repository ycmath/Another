...
code = gzip.decompress(base64.b64decode(raw)).decode()
old = "m.zero_();po.zero_();p.zero_()"
new = "m.fill(0);po.fill(0);p.fill(0)"
if old not in code:
    raise RuntimeError("expected IPC harness patch target not found")
code = code.replace(old, new, 1)
path = "/tmp/run_x5v_compact.py"
open(path, "w", encoding="utf-8").write(code)
print("STAGED_SCRIPT_BYTES", len(code.encode()), "PATCHED_NUMPY_ZERO", flush=True)
